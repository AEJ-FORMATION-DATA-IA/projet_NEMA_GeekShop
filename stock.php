<?php
    ...
    $referenceError = $libelleError = $quantite_minimaleError = $quantite_a_stockerError = $reference = $libelle = $quantite_minimale = $stock = $newStock = $quantite_en_stock = "";
    ...
    if(!empty($_POST))
    {
        ...
        $newStock = checkInput($_POST['quantite_a_stocker']);
        $isSuccess         = true;

        $db = Database::connect();
        $statement = $db->prepare("SELECT quantite_en_stock FROM produit WHERE id = ?");
        $statement->execute(array($id));
        $item = $statement->fetch();
        $quantite_en_stock = $item['quantite_en_stock'];
        $stock = (int)$newStock + (int)$quantite_en_stock;
        Database::disconnect();
...

        {
            $db = Database::connect();
                ...
                $statement->execute(array($reference,$libelle,$quantite_minimale,$stock,$id));              
    ...
    <div class="container admin">
            <div class="row">
                    <h1><strong>Modifier l'enregistrement du produit</strong></h1>
                        <br>
                        <form action="<?php echo 'stock.php?id=' .$id; ?>" role="form" method="post" class="form">
                                    <div class="form-group">
                                        <label for="reference">Reference : </label>
                                        <input type="text" readonly class="form-control" name="reference" id="reference" placeholder="Reference du produit" value="<?php echo $reference; ?>"> 
                                    ...
                                    <div class="form-group">
                                        <label for="quantite_a_stocker" class="alert alert-danger">Quantite à stocker : </label>
                                        <input type="number" class="form-control alert alert-warning" name="quantite_a_stocker" id="quantite_a_stocker" placeholder="La quantite à ajouter dans le stock "> 
                                        <span class="help-inline"><?php echo $quantite_a_stockerError; ?></span>
                                    </div>
